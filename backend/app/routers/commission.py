...
def get_commission_config(id: int, db: Session = Depends(get_db), current_user: CurrentUser = Depends(get_current_user)):
...
def get_current_effective_config(db: Session = Depends(get_db), current_user: CurrentUser = Depends(get_current_user)):
...
def create_commission_config(payload: CommissionConfigCreate, db: Session = Depends(get_db), current_user: CurrentUser = Depends(get_current_user)):
...
def update_commission_config(payload: CommissionConfigUpdate, db: Session = Depends(get_db), current_user: CurrentUser = Depends(get_current_user)):
...
def delete_commission_config(id: int, db: Session = Depends(get_db), current_user: CurrentUser = Depends(get_current_user)):
...
def get_agent_fee(id: int, db: Session = Depends(get_db), current_user: CurrentUser = Depends(get_current_user)):
...
def create_agent_fee(payload: AgentFeeCreate, db: Session = Depends(get_db), current_user: CurrentUser = Depends(get_current_user)):
    obj = svc.create_agent_fee(db, payload)
    return ApiResponse(message="Created", data={"id": obj.id})


@router.put("/agent-fee/update", response_model=ApiResponse)
def update_agent_fee(payload: AgentFeeUpdate, db: Session = Depends(get_db), current_user: CurrentUser = Depends(get_current_user)):
...
def delete_agent_fee(id: int, db: Session = Depends(get_db), current_user: CurrentUser = Depends(get_current_user)):
...
def batch_delete_agent_fees(ids: list[int] = Query(...), db: Session = Depends(get_db), current_user: CurrentUser = Depends(get_current_user)):
...
def calculate_and_generate_agent_fee(payload: AgentFeeCalculate, db: Session = Depends(get_db), current_user: CurrentUser = Depends(get_current_user)):
...
def approve_agent_fee(payload: AgentFeeApproval, db: Session = Depends(get_db), current_user: CurrentUser = Depends(get_current_user)):
...
def batch_approve_agent_fees(payload: AgentFeeBatchApproval, db: Session = Depends(get_db), current_user: CurrentUser = Depends(get_current_user)):
...
def batch_settle_agent_fees(payload: AgentFeeBatchSettlement, db: Session = Depends(get_db), current_user: CurrentUser = Depends(get_current_user)):
